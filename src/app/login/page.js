...
// Icons
const MailIcon = () => (<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" /></svg>);
  ...
  return (
    <div className="min-h-screen bg-[#020202] flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-yellow-500/30">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-black/40 backdrop-blur-3xl border border-white/5 rounded-[2rem] p-8 shadow-[0_0_100px_-20px_rgba(255,255,255,0.05)] relative z-10"
      >
        ...
        <div className="text-center mb-10 mt-2">
          <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-white/40 tracking-tight mb-2">Welcome Back</h2>
          <p className="text-gray-500 text-sm">Access your personalized learning dashboard.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-gray-500 ml-1 uppercase tracking-wider">Email</label>
            <div className={`group flex items-center bg-white/5 border rounded-xl px-4 py-3.5 transition-all focus-within:bg-white/10 focus-within:border-yellow-500/50 ${errors.email ? "border-red-500/50" : "border-white/5"}`}>
          ...
          <div className="space-y-1">
            ...
            <div className={`group flex items-center bg-white/5 border rounded-xl px-4 py-3.5 transition-all focus-within:bg-white/10 focus-within:border-yellow-500/50 ${errors.password ? "border-red-500/50" : "border-white/5"}`}>
          ...
          <div className="flex justify-end">
            <Link href="#" className="text-xs text-gray-400 hover:text-white transition-colors">Forgot Password?</Link>
          </div>

          <motion.button whileHover={{ scale: 1.01, boxShadow: "0 0 20px rgba(234, 179, 8, 0.3)" }} whileTap={{ scale: 0.99 }} type="submit" disabled={loading} className="w-full bg-yellow-500 text-black font-bold py-4 rounded-xl shadow-lg transition-all relative overflow-hidden">
        ...
        <div className="mt-8 text-center">
          <p className="text-gray-500 text-sm">Don't have an account? <Link href="/signup" className="text-yellow-500 font-semibold hover:text-yellow-400 transition-colors">Create Account</Link></p>
        </div>
      </motion.div>
    </div>
  );
