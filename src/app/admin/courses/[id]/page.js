...
import { 
  ...
  Users, IndianRupee, Clock, Wallet, Search, UserX, AlertCircle, UserPlus, CreditCard
} from "lucide-react";
...
export default function CourseDetailsPage() {
  ...
  // NEW: State to track which student is being removed (Loading State)
  ...
  const handleRemoveStudent = async (studentId) => {
    ...
    setRemovingId(studentId); // Start Loading for this specific button
    ...
    try {
      ...
      if (res.ok) {
        ...
        // alert("Student removed successfully"); // Alert hataya clean UX ke liye
      } else {
      ...
      setRemovingId(null); // Stop Loading
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white pb-20 relative">
       <div className={`absolute top-0 left-0 w-full h-[40vh] bg-gradient-to-b ${course.gradient} opacity-20 blur-3xl pointer-events-none`}></div>
       ...
       <div className="max-w-7xl mx-auto p-4 md:p-8 relative z-10">
          ...
          <div className="flex flex-col md:flex-row gap-6 md:items-end justify-between mb-10">
             <div>
                <span className="px-3 py-1 rounded-full bg-white/10 text-yellow-400 text-xs font-bold uppercase tracking-widest mb-3 inline-block">
                    {course.category}
                </span>
                <h1 className="text-3xl md:text-5xl font-black tracking-tight mb-2">{course.title}</h1>
                <p className="text-gray-400 max-w-2xl text-sm md:text-base">{course.description}</p>
             </div>
             <div className="flex items-center gap-3">
                <button 
                  onClick={() => setIsEditOpen(true)}
                  className="flex items-center gap-2 px-5 py-2.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl font-bold text-sm transition-all"
                >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-[#111] border border-white/10 p-5 rounded-2xl flex flex-col items-center justify-center text-center gap-2 hover:border-blue-500/30 transition-colors group">
                ...
                <div>
                    ...
                    <p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold tracking-wider">Enrolled</p>
                </div>
              </div>
              <div className="bg-[#111] border border-white/10 p-5 rounded-2xl flex flex-col items-center justify-center text-center gap-2 hover:border-green-500/30 transition-colors group">
                <IndianRupee className="text-green-400 group-hover:scale-110 transition-transform" size={24} />
                <div>
                    <p className="text-2xl font-bold">{course.price}</p>
                    <p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold tracking-wider">Price</p>
                </div>
              </div>
              <div className="bg-[#111] border border-white/10 p-5 rounded-2xl flex flex-col items-center justify-center text-center gap-2 hover:border-purple-500/30 transition-colors group">
                ...
                <div>
                    ...
                    <p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold tracking-wider">Duration</p>
                </div>
              </div>
              <div className="bg-[#111] border border-white/10 p-5 rounded-2xl flex flex-col items-center justify-center text-center gap-2 hover:border-yellow-500/30 transition-colors group">
                <Wallet className="text-yellow-400 group-hover:scale-110 transition-transform" size={24} />
                <div>
                    <p className="text-2xl font-bold">₹{(course.price * course.students).toLocaleString()}</p>
                    <p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold tracking-wider">Revenue</p>
                </div>
              </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             ...
             <div className="md:col-span-2 space-y-6">
                ...
                {/* Control Panel (Moved Up as requested) */}
                <div className="bg-[#111] border border-white/10 rounded-3xl p-6 md:p-8">
                    <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">Course Controls</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl bg-white/5 border border-white/5 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">{course.isActive ? "Visible on app." : "Hidden from users."}</p>
                            </div>
                            <button 
                                onClick={() => handleToggle('isActive', !course.isActive)}
                                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${course.isActive ? 'bg-green-500/20 text-green-400 hover:bg-green-500/30' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                            >
                        ...
                        <div className="p-4 rounded-xl bg-white/5 border border-white/5 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">{course.isLocked ? "Closed." : "Open."}</p>
                            </div>
                            <button 
                                onClick={() => handleToggle('isLocked', !course.isLocked)}
                                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${course.isLocked ? 'bg-red-500/20 text-red-400 hover:bg-red-500/30' : 'bg-blue-500/20 text-blue-400 hover:bg-blue-500/30'}`}
                            >
                ...
                {/* Enrolled Students Section (Stable Button) */}
                            ...
                            <table className="w-full text-left text-sm">
                                <thead className="text-xs text-gray-500 uppercase bg-white/5 sticky top-0 backdrop-blur-md">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 font-bold hidden md:table-cell">Details</th>
                                    ...
                                    {filteredStudents.map((student) => (
                                        <tr key={student._id} className="hover:bg-white/[0.02] transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                {/* FIXED BUTTON: No Hover Shift, Loading State Added */}
                                                <button 
                                                    onClick={() => handleRemoveStudent(student._id)}
                                                    disabled={removingId === student._id}
                                                    className="px-4 py-2 bg-red-500/10 hover:bg-red-500 hover:text-white text-red-500 border border-red-500/20 text-xs font-bold rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 ml-auto"
                                                >
             ...
             {/* Right Column: Manual Actions & Danger Zone */}
             <div className="space-y-6">
                
                {/* Manual Enroll */}
                <div className="bg-gradient-to-b from-[#1a1a1a] to-black border border-white/10 rounded-3xl p-6 relative overflow-hidden">
                    ...
                    <form onSubmit={handleManualEnroll} className="space-y-4 relative z-10">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-yellow-400 outline-none transition-all"
                            />
                        </div>
                        ...
                        <button 
                            ...
                            className="w-full py-3 bg-yellow-400 hover:bg-yellow-300 text-black font-bold rounded-xl shadow-lg shadow-yellow-400/20 active:scale-95 transition-all flex justify-center items-center gap-2"
                        >
                ...
                {/* Course ID Info */}
                <div className="p-5 rounded-3xl bg-[#111] border border-white/10 space-y-4">
                    ...
                    <div className="p-3 bg-black/50 rounded-xl border border-white/5">
                        ...
                        <p className="text-xs text-gray-300 mt-1">{new Date(course.updatedAt).toLocaleDateString()}</p>
                    </div>
                </div>
                ...
                <div className="bg-red-900/10 border border-red-500/20 rounded-3xl p-6">
                    ...
                    <button 
                        ...
                        className="w-full py-2.5 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl transition-all text-xs font-bold border border-red-500/20"
                    >
                        Delete Permanently
                    </button>
                </div>
