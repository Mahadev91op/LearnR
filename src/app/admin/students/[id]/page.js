...
import { 
  ...
  BookOpen, School, User, Hash, GraduationCap, Shield, AlertCircle, Edit2, Save, X, Power
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-3 pb-24 md:p-8 relative overflow-y-auto overflow-x-hidden selection:bg-yellow-500/30">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="sticky top-0 z-50 md:static flex justify-between items-center mb-6 md:mb-8 bg-[#050505]/80 md:bg-transparent backdrop-blur-md md:backdrop-blur-none py-3 md:py-0 -mx-3 px-3 md:mx-0 md:px-0 border-b border-white/5 md:border-none">
            <button 
              onClick={() => router.back()} 
        ...
        {/* Profile Header Card - Compact on Mobile */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          {/* Left Column: Personal Details (Spans 2 cols on Desktop, 1 on Mobile/Tablet) */}
          ...
          {/* Right Column: Account Summary & Status (Full width on Mobile/Tablet, Side on Desktop) */}
            ...
            <div className={`bg-gradient-to-b from-white/[0.05] to-transparent border border-white/10 rounded-2xl md:rounded-3xl p-5 md:p-6 transition-colors ${isEditing ? 'border-yellow-500/20' : ''}`}>
              ...
              {isEditing && (
                <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-xl">
                    <p className="text-[10px] md:text-xs text-yellow-200 flex gap-2">
                        <AlertCircle size={14} className="shrink-0 mt-0.5" />
                        Saving changes will send an email notification.
                    </p>
                </div>
              )}
            </div>
...
// Helper Component for Details (Handles both View and Edit modes)
