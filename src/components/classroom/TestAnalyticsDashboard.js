...
export default function TestAnalyticsDashboard({ testId, onBack }) {
  ...
  if (loading) return <div className="p-20 text-center text-yellow-500 font-bold animate-pulse">ANALYZING EXAM DATA...</div>;
  if (!data) return <div className="p-20 text-center text-red-500">Failed to load analytics.</div>;
                  ...
                  {data.analytics.hardQuestions.map((q, i) => (
                      <div key={i} className="space-y-1">
                          ...
                          <div className="h-1.5 w-full bg-gray-800 rounded-full overflow-hidden">
                              <div 
                                style={{width: `${(q.wrongCount / data.analytics.stats.present) * 100}%`}} 
          ...
          <div className="bg-[#0a0a0a] border border-blue-500/20 rounded-2xl p-6 relative overflow-hidden">
              ...
              <div className="mt-8">
                 <div className="h-2 w-full bg-gray-800 rounded-full flex overflow-hidden">
                    <div style={{width: `${(data.analytics.stats.present/data.analytics.stats.totalStudents)*100}%`}} className="bg-green-500 h-full"></div>
                    <div style={{width: `${(data.analytics.stats.absent/data.analytics.stats.totalStudents)*100}%`}} className="bg-red-500 h-full"></div>
                 </div>
              </div>
          </div>
      </div>
                ...
                <tbody className="divide-y divide-white/5">
                    {filteredStudents.map((s) => (
                        <tr key={s.id} className="hover:bg-white/5 transition-colors group">
                            ...
                            <td className="p-4">
                                <p className="font-bold text-white group-hover:text-yellow-400 transition-colors">{s.name}</p>
