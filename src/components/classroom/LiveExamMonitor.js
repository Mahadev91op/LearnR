...
export default function LiveExamMonitor({ testId, onBack }) {
  ...
  const fetchLiveData = async () => {
    try {
      ...
      const analyticsRes = await fetch(`/api/admin/tests/${testId}/analytics`);
      if (analyticsRes.ok) {
  ...
  useEffect(() => {
    fetchLiveData();
    const interval = setInterval(fetchLiveData, 15000); // Auto-refresh data every 15s
    return () => clearInterval(interval);
  }, [testId]);

  // 2. LIVE COUNTDOWN LOGIC (Updated for 12 Hours Window)
  useEffect(() => {
    ...
    const calculateTime = () => {
      ...
      // LOGIC CHANGE: 12 Hours Window (in milliseconds)
      const windowDurationMs = 12 * 60 * 60 * 1000; 
      
      const endTime = startTime + windowDurationMs; // Window closes 12 hours after start
      const now = new Date().getTime();
      
      const difference = endTime - now;
      
      // Calculate Progress Percentage based on 12 hours
      ...
      // Critical Alert if less than 30 mins left in the 12-hour window
      if (difference < 30 * 60 * 1000) setIsTimeCritical(true);
      else setIsTimeCritical(false);
    };

    calculateTime(); // Run immediately
    const timerInterval = setInterval(calculateTime, 1000); // Run every second
  ...
  const handleEndExam = async () => {
    if (!confirm("Are you sure you want to END this exam for everyone? Students won't be able to submit anymore.")) return;
  ...
  if (loading && !testDetails) {
    return <div className="flex h-96 items-center justify-center"><Loader2 className="animate-spin text-cyan-500" size={40}/></div>;
  }

  // Calculate Display Times
  const startTimeObj = testDetails ? new Date(testDetails.scheduledAt) : new Date();
  // End Time is strictly Start + 12 Hours
    ...
    <div className="animate-in fade-in slide-in-from-right duration-300 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pb-6 border-b border-white/10">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
            <button onClick={fetchLiveData} className="p-3 bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 hover:text-white transition-all" title="Refresh Data">
                <RefreshCcw size={20} />
            </button>
            <button 
                onClick={handleEndExam} 
                disabled={endingExam}
                className="flex-1 md:flex-none justify-center flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl transition-all shadow-[0_0_20px_rgba(220,38,38,0.4)] hover:shadow-[0_0_30px_rgba(220,38,38,0.6)] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
          ...
          {/* 1. TIMER CARD (Tracks 12 Hour Window) */}
          <div className={`md:col-span-4 relative overflow-hidden rounded-2xl border ${isTimeCritical ? 'border-red-500/30 bg-red-500/5' : 'border-cyan-500/30 bg-cyan-500/5'} p-6 flex flex-col justify-center items-center text-center shadow-[0_0_30px_-10px_rgba(0,0,0,0.5)]`}>
              ...
              <p className="text-gray-500 text-xs mt-4">Accepting answers till {windowEndTimeObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
          </div>
              ...
              <div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-center">
                          <span className="text-gray-500 text-sm">Student Duration</span>
          ...
          <div className="md:col-span-4 bg-[#0a0a0a] border border-white/5 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 bg-[#111] rounded-xl p-4 flex justify-between items-center border border-white/5">
                      <span className="text-gray-400 text-xs font-bold uppercase">Average Score</span>
                      <span className="text-xl font-black text-yellow-400">{analytics?.averageScore ? analytics.averageScore.toFixed(1) : "0.0"}</span>
                  </div>
              </div>
          </div>
      </div>
          ...
          <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="relative w-full md:w-64">
                  ...
                  <input 
                    ...
                    className="w-full bg-[#111] border border-white/10 rounded-lg py-2 pl-9 pr-4 text-sm text-white focus:border-cyan-500 outline-none transition-colors"
                  />
              </div>
          </div>
