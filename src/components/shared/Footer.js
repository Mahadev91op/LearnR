...
const SocialIcon = ({ path, href, label }) => (
  <motion.a
    ...
    whileTap={{ scale: 0.95 }}
    className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-gray-400 hover:text-yellow-400 hover:border-yellow-500/50 hover:bg-yellow-500/10 transition-colors"
  >
...
const FooterLink = ({ href, children }) => (
  <li>
    <Link href={href} className="group flex items-center text-sm text-gray-400 hover:text-yellow-400 transition-colors duration-300">
  ...
  return (
    <div className="border-b border-white/10 overflow-hidden">
      <button 
        ...
        className="w-full flex items-center justify-between py-4 text-left text-white font-medium active:bg-white/5 transition-colors"
      >
        <span>{title}</span>
        <motion.span 
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-8 mb-16">
            ...
            <motion.div initial="hidden" whileInView="visible" variants={containerVariants} className="md:col-span-2 space-y-4">
              <h3 className="text-white font-bold text-lg tracking-wide">Platform</h3>
            ...
            <motion.div initial="hidden" whileInView="visible" variants={containerVariants} className="md:col-span-2 space-y-4">
              <h3 className="text-white font-bold text-lg tracking-wide">Resources</h3>
              <ul className="space-y-3">
                <ResourceLinksList />
              </ul>
            </motion.div>
...
function NewsletterForm({ mobile }) {
    return (
        <>
            {!mobile && (
                <p className="text-gray-400 text-sm mb-4">
                  Subscribe to get the latest English tips and exclusive offers.
                </p>
            )}
            <form className="relative group">
                <div className="absolute inset-0 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-lg blur opacity-20 group-hover:opacity-40 transition-opacity"></div>
                <div className="relative flex bg-gray-900 border border-white/10 rounded-lg overflow-hidden p-1 focus-within:border-yellow-500/50 transition-colors">
                  <input 
                    type="email" 
                    placeholder="Enter your email" 
                    className="w-full bg-transparent text-white px-4 py-2 outline-none placeholder:text-gray-600 text-sm"
                  />
                  <motion.button 
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    type="submit"
                    className="bg-yellow-500 text-black font-bold px-4 md:px-6 py-2 rounded-md text-sm hover:bg-yellow-400 transition-colors whitespace-nowrap"
                  >
                    Join
                  </motion.button>
                </div>
